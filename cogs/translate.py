...
class Translate(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        # Dosyandaki dil ID'lerini buraya taşıdık
        self.LANG_ROLES = {
    ...
    async def on_message(self, message):
        ...
        if message.author.bot or not message.guild or not message.content or message.content.startswith("!"):
            return
        
        try:
            await message.add_reaction("🌐")
        except Exception as e:
            print(f"Reaction hatası: {e}")
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        # Kullanıcının dil rolünü bul
        lang = "en"
        for role in user.roles:
            if role.id in self.LANG_ROLES:
                lang = self.LANG_ROLES[role.id]
                break
